    ...
    }else if (moduloAtual == mod_curvas)
{
    if (modoCurvaAtual == modo_curva_1d)
    {
        ...
        renderizarTexto(LARGURA_PAINEL_LATERAL + 15, alturaJanela - 40,
            "Mouse: mover pontos | G R M C +/- [ ] | 1-4 presets | B: superficie",
            0.45f,0.45f,0.5f);
    }
    ...
    else if (modoCurvaAtual == modo_revolucao)
    {
        ...
        renderizarTexto(LARGURA_PAINEL_LATERAL + 15, alturaJanela - 40,
            "UP/DOWN: fatias | Mouse: editar perfil | C | 1-4 | V: curva",
            0.45f,0.45f,0.5f);
...
void mouseClick(int botao, int estado, int x, int y) {
    ...
    eixosProjecoes.zoom=zoom;
     ...
     if (moduloAtual == mod_curvas && botao == GLUT_LEFT_BUTTON) {
        // Seleção de ponto só funciona nos modos que exibem pontos
        if (modoCurvaAtual == modo_curva_1d ||
            modoCurvaAtual == modo_revolucao) {
            if (estado == GLUT_DOWN) {
                int xViewport = x - LARGURA_PAINEL_LATERAL;
                int yViewport = y;
                selecionarPontoControle(xViewport, yViewport,
                    larguraJanela - LARGURA_PAINEL_LATERAL, alturaJanela);
            } else {
                soltarPontoControle();
            }
        }
...
void mouseMove(int x, int y) {
    ...
    if (moduloAtual == mod_curvas && botaoPressionado == GLUT_LEFT_BUTTON
        && curvaAtual.pontoSelecionado >= 0) {

        int xViewport = x - LARGURA_PAINEL_LATERAL;
        int yViewport = y;
        arrastarPontoControle(xViewport, yViewport,
            larguraJanela - LARGURA_PAINEL_LATERAL,
            alturaJanela  - ALTURA_PAINEL_RODAPE);

    } else if (botaoPressionado == GLUT_LEFT_BUTTON && x > LARGURA_PAINEL_LATERAL) {
        // Rotação normal da câmera (código existente)
        anguloY += (x - mouseX) * 0.5f;
        anguloX += (y - mouseY) * 0.5f;
        eixosTransformacoes.xrotate = anguloX;
        eixosTransformacoes.yrotate = anguloY;
    ...
    if (moduloAtual == mod_iluminacao) processarTecladoIluminacao(key);
     if (moduloAtual == mod_curvas) {
        ...
        if (modoCurvaAtual == modo_curva_1d ||
            ...
            // Presets de formas (teclas 1-4)
            if (key == '1') aplicarPreset(preset_S);
            if (key == '2') aplicarPreset(preset_arco);
            if (key == '3') aplicarPreset(preset_onda);
            if (key == '4') aplicarPreset(preset_diagonal);
        }
...
void init() {
    ...
    glEnable(GL_DEPTH_TEST);
    curvaAtual.numSegmentos = 30;
    curvaAtual.mostrarPoligono = true;
