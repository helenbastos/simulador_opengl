...
void alternarPreset(int direcao) {
    presetAtualIdx += direcao;
    if (presetAtualIdx >= NUM_PRESETS) presetAtualIdx = 0;
    if (presetAtualIdx < 0) presetAtualIdx = NUM_PRESETS - 1;
    
    aplicarPreset((PresetCurva)presetAtualIdx);
    glutPostRedisplay();
}
...
void desenharSuperficieBezier() {
    ...
    // Configura o evaluator 2D
    // Parâmetros: tipo, uMin, uMax, passo_u (stride), ordem_u,
...
void preencherPainelCurvas() {
    ...
    case modo_curva_1d:
        ...
        if (!curvaAtual.modoRender) {
            ...
            adicionarLinhaAtalho("Setas Esq/Dir");
